...
def edges(points):
    ...
    if dimension == 2:
        ...
        H_edges = sorted([tuple(sorted((sorted_indices1[i], sorted_indices1[j]))) for i, j in res1])
        ...
        V_edges = sorted([tuple(sorted((sorted_indices2[i], sorted_indices2[j]))) for i, j in res2])
